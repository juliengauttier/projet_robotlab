...
start() ->
	...
	register(salle1,spawn(projet,salle,[salle1,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle2,spawn(projet,salle,[salle2,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle3,spawn(projet,salle,[salle3,[],[true,true,true,true],1,[1,1,1,1],false,false])), 

	register(salle4,spawn(projet,salle,[salle4,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle5,spawn(projet,salle,[salle5,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle6,spawn(projet,salle,[salle6,[],[true,true,true,true],1,[1,1,1,1],false,false])), 

	register(salle7,spawn(projet,salle,[salle7,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle8,spawn(projet,salle,[salle8,[],[true,true,true,true],1,[1,1,1,1],false,false])), 
	register(salle9,spawn(projet,salle,[salle9,[],[true,true,true,true],1,[1,1,1,1],false,false])),
	   ...
	   register(robot1,spawn(projet,robot,[1,salle1,0])),
		%register(robot2,spawn(projet,robot,[2,salle2,0])),
	   register(robot3,spawn(projet,robot,[3,salle3,0])),
	   %register(robot5,spawn(projet,robot,[5,salle5,0])),
	   register(robot7,spawn(projet,robot,[7,salle7,0])),
	   %register(robot8,spawn(projet,robot,[8,salle8,0])),
...
unregisterIfPresentInListAux(Proc,[Proc|_])->
	unregister(Proc),
	ok;
unregisterIfPresentInListAux(Proc,[_|RProcList])->
	unregisterIfPresentInListAux(Proc,RProcList).
	
	


finDetection(0,_,_)->ok;
finDetection(N,[],ParoleReservee)->
	receive
		{demandePermissionParler,RobotProc,_,_}->
			if
				ParoleReservee->
...
choisirChemin(RobotNum,AncienneSalle,Pos,_,[])->
	robotDemandeChemin(RobotNum, AncienneSalle, Pos);
	
choisirChemin(RobotNum,AncienneSalle,Pos,_,[{salle0,PosToGo}|_])->
					faireTraverserRobot(RobotNum,Pos,PosToGo,salle0),
...
faireTraverserRobot(RobotNum,PosToGo,PosToGo,_)->
	R=robotlab:get_robot(RobotNum),
	robotlab:porte(R),
	robotlab:franchit(R),
	if
...
porteVersVoisinCommunicant(_,[],[])->false;
porteVersVoisinCommunicant(VCommunication,[VCommunication|_],[true|_])->true;
porteVersVoisinCommunicant(VCommunication,[_|VR],[_|PR])->porteVersVoisinCommunicant(VCommunication,VR,PR).

cheminSortie(V,P)->cheminSortieAux(V,P,3).
cheminSortieAux([],[],_)->[];
cheminSortieAux([salle0|_],[true|_],Pos)->[{salle0,Pos}];
cheminSortieAux([_|VR],[_|PR],Pos)->
	if
		Pos == 3 ->
			NewPos = 0;
		true ->
...
getCheminAux([_|VR],[_|PR],[_|DR],DistanceRecherche,Pos)->
	if
		Pos == 3 ->
			NewPos = 0;
		true ->
...
reatribDistVoisin([],[],[])->[];
reatribDistVoisin([_|VR],[false|PR],[_|DR])->[100|reatribDistVoisin(VR,PR,DR)];
reatribDistVoisin([salle0|VR],[true|PR],[_|DR])->[-100000|reatribDistVoisin(VR,PR,DR)];
reatribDistVoisin([_|VR],[true|PR],[D|DR])->[D|reatribDistVoisin(VR,PR,DR)].
...
minDistVoisinAux([false|PR],[_|DR],Min)->minDistVoisinAux(PR,DR,Min).
...
guessNewDist([_|VR],[true|PR],[D|DR],MinDist)->
	   if
			  D < MinDist ->
...
guessNewDist([_|VR],[false|PR],[_|DR],MinDist)->guessNewDist(VR,PR,DR,MinDist);
...
sendNotifVoisinUpdateAux(Nom,[],[],[],_)->
	mainproc ! {communicationSalleFin,Nom},
	[];
sendNotifVoisinUpdateAux(Nom,[_|PR],[salle0|VR],[D|DR],NewDist)->
...
sendNotifVoisinUpdateAux(Nom,[true|PR],[V|VR],[D|DR],NewDist)->
	   V ! {miseAJourDistanceVoisin,Nom,self(),NewDist},
		receive 
			{acquittementNotif,V}->
				MajDist = D;
			{miseAJourDistanceVoisin,V,_,DistCom}->
				MajDist = DistCom,
				V ! {acquittementNotif,Nom}
		end,
	   [MajDist | sendNotifVoisinUpdateAux(Nom,PR,VR,DR,NewDist)];
sendNotifVoisinUpdateAux(Nom,[false|PR],[_|VR],[D|DR],NewDist)->
	[D|sendNotifVoisinUpdateAux(Nom,PR,VR,DR,NewDist)].


majDistanceVoisin([ProcSalleCom|VR],[_|DR],ProcSalleCom,DistCom)->[DistCom|majDistanceVoisin(VR,DR,ProcSalleCom,DistCom)];
majDistanceVoisin([_|VR],[DistancesOld|DR],ProcSalleCom,DistCom)->[DistancesOld|majDistanceVoisin(VR,DR,ProcSalleCom,DistCom)];
majDistanceVoisin([],[],_,_)->[].
